...
const ProductHero = ({ onOpenPilot }) => {
  ...
  return (
    <section ref={heroRef} className="relative min-h-0 sm:min-h-screen w-full bg-[#07080a] text-white pt-28 sm:pt-36 pb-12 sm:pb-20 px-4 sm:px-12 flex flex-col justify-between overflow-hidden">
      ...
      <div ref={bgRef} className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] sm:w-[700px] h-[400px] sm:h-[700px] bg-rose-500/10 rounded-full blur-[160px] pointer-events-none" />

      {/* Header Block */}
      <div className="max-w-7xl mx-auto w-full z-10 text-center flex flex-col items-center">
        {/* Category Pill */}
        <div ref={badgeRef} className="pill-badge mb-4 sm:mb-6 border-rose-500/30 bg-rose-500/10 text-rose-400">
        ...
        <div ref={titleRef} className="space-y-2 sm:space-y-3 max-w-4xl">
          <h1 className="font-display text-4xl sm:text-7xl md:text-8xl lg:text-[7.5rem] font-bold tracking-tight uppercase leading-[0.95]">
        ...
        <div ref={ctaRef} className="mt-6 sm:mt-8 flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
          <button
            onClick={onOpenPilot}
            className="px-7 sm:px-8 py-3.5 sm:py-4 rounded-full bg-white text-black font-semibold text-xs uppercase tracking-wider flex items-center gap-2 hover:bg-rose-500 hover:text-white transition-all duration-300 shadow-2xl group"
          >
          ...
          <a
            href="#product-specs"
            className="px-6 py-3 sm:py-3.5 rounded-full border border-white/20 text-white text-xs font-mono uppercase tracking-wider hover:bg-white/10 transition"
          >
            View Specifications
          </a>
        </div>
      ...
      <div className="max-w-6xl mx-auto w-full z-10 mt-8 sm:mt-12 mb-6 sm:mb-8">
        <div ref={imageRef} className="relative rounded-2xl sm:rounded-3xl overflow-hidden border border-white/10 bg-[#0c0d10] shadow-2xl group">
          ...
          <div ref={selectorRef} className="absolute bottom-4 left-4 right-4 sm:bottom-8 sm:left-8 sm:right-8 flex flex-wrap items-center justify-between gap-3 sm:gap-4 glass-card p-3 sm:p-6 rounded-xl sm:rounded-2xl border border-white/10">
            <div className="flex flex-wrap gap-3 sm:gap-4">
              {specPills.map((pill, i) => {
                const Icon = pill.icon;
                return (
                  <div key={i} className="flex items-center gap-2 font-mono text-[10px] sm:text-xs text-slate-300">
                    <Icon className="text-rose-400 text-xs sm:text-sm" />
                    <span>{pill.label}</span>
                  </div>
                );
              })}
            </div>
            <span className="font-mono text-[9px] sm:text-[10px] text-rose-400 uppercase tracking-widest border border-rose-500/30 px-2.5 py-0.5 rounded-full">
              PATENT PENDING
            </span>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="z-10 flex flex-col items-center gap-2 pt-2">
        <a href="#product-intro" className="flex items-center gap-2 font-mono text-[10px] sm:text-[11px] text-slate-500 uppercase tracking-widest hover:text-slate-300 transition">
          <span>EXPLORE</span>
