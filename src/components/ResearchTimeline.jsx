...
const ResearchTimeline = () => {
  ...
  return (
    <section id="research" ref={sectionRef} className="relative w-full bg-[#0c0d10] text-white py-32 px-6 sm:px-12 border-t border-white/10">
      <div className="max-w-7xl mx-auto space-y-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {timelineSteps.map((step, idx) => (
            <div key={idx} className="glass-card rounded-3xl p-6 border border-white/10 space-y-4 relative group hover:border-rose-500/40 transition">
              <div className="flex items-center justify-between">
                <span className="font-mono text-xs font-bold text-rose-500">{step.month}</span>
                <span className="font-mono text-[10px] text-slate-400 uppercase border border-white/10 px-2 py-0.5 rounded-full">
                  {step.badge}
                </span>
              </div>
        ...
        <div className="p-8 sm:p-12 rounded-3xl bg-[#07080a] border border-white/10 space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
            <div>
              <span className="font-mono text-xs text-rose-400 uppercase tracking-widest block">DATA FOUNDATION</span>
              <h3 className="font-display text-2xl font-bold text-white">Multi-Source Clinical Dataset</h3>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {dataPillars.map((dp, i) => (
              <div key={i} className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-2">
                <FiDatabase className="text-rose-400 text-xl" />
                <h4 className="font-display text-sm font-bold text-white">{dp.title}</h4>
                <p className="text-xs text-slate-400 font-mono">{dp.source}</p>
              </div>
            ))}
          </div>

          <div className="pt-4 text-center font-mono text-xs text-slate-300 uppercase tracking-widest border-t border-white/10">
